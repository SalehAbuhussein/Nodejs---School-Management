 ...
 */
export const getExamType = async (examTypeId: string): Promise<IExamType | null> => {
  try {
 ...
 */
export const updateExamType = async (examTypeId: string, examTypeData: IExamType): Promise<IExamType | null> => {
  try {
    const examType = await ExamType.findById(examTypeId);
